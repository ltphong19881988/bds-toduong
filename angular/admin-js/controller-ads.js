...
// ==================== slider Controller ================
var getAllAds = function($scope, $http) {
    let params = {
        method: 'POST',
        url: '/admin/ads/all',
    }
    submitBackend(params, $http, function(res) {
        console.log('all ads', res);
        $scope.listAds = res;
    });
}

adminApp.controller("adsCtrl", function($rootScope, $scope, $compile, $http, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder) {
    ...
    $scope.unitChanged = function(){
        if($scope.item.position == '') delete $scope.item.position;
        console.log($scope.item);
    }

    $scope.submitAddAds = function() {
    ...
    $scope.submitUpdateAds = function(e) {
        jQuery('#loadingModalTitle').html('Loading ....');
        jQuery('#loadingModal').modal('show');
        // console.log($scope.item);
        var params = {
